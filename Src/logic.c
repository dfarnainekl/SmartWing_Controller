...
void logic_init(void)
{
	...
	for(i=0; i<4; i++) TMC4671_highLevel_positionMode(i); // TODO: start one after the other with soft flux/torque ramp
...
void logic_loop(void)
{
	...
	if(systick_counter_2 >= 500) //2Hz
	{
		systick_counter_2 = 0;

		//FIXME: print does not work correctly when float calculation is enabled above
//		char string[128];
	...
	if(htim == &htim2) // pwm in
	{
		...
		switch(htim->Channel)
		{
			case HAL_TIM_ACTIVE_CHANNEL_1:
				if(HAL_GPIO_ReadPin(PWM_IN_0_GPIO_Port, PWM_IN_0_Pin)) timestamp_risingEdge[0] = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1); //rising edge
